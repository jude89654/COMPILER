	...
	public Token nextToken() {
		...
		while (true) {
			...
			case "Qstart":
				...
				case ';':
					currentCharacter = read();
					return new Token("SEMICOLON", ";", line, Token.SEMICOLON);
				case ',':
					currentCharacter = read();
			...
			case "QnumStart":
				if (isNumeric(currentCharacter)) {
					...
					state = "QidStart"; // doesnot start with number or symbol
										// go to id
				}
				continue;
			...
			case "Qfloat":
				if (isNumeric(currentCharacter)) {
					...
					return new Token("ERROR", "Invalid input: " + mgaNabasangNumberNaSunodSunod + ".", line,
							Token.T_UNKNOWN);
				}
				continue;
				// decimal body
			case "QFloatBody":
				if (isNumeric(currentCharacter)) {
					...
					return new Token("NUMDEC", "" + mgaNabasangNumberNaSunodSunod + "." + decBuffer, line,
							Token.NUMDEC);
				}
				continue;
			...
			case "Q#":
				...
				if (currentCharacter == '\n') {
					...
					currentCharacter = read();
					state = "Qstart";
					// return new Token("Comment", "#" + mgaNabasangLetra, line
					// - 1,Token.COMMENT);
